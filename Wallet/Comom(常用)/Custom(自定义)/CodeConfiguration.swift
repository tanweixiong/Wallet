...
class CodeConfiguration: NSObject {
    ...
    class func codeProcessing(_ vc:UIViewController,_ data:NSArray,success : @escaping (_ response : String,_ type : String,_ data:String)->()){
        ...
        //首页
        if type == "1" {
            success(address,type,businesscard)
        //转账
        }else if type == "2"{
            success(address,type,businesscard)
        //名片 直接添加好友
